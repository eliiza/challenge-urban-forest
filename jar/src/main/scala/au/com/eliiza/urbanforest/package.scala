...
package object urbanforest {
  ...
  type Loop = Seq[Point]
  type Polygon =
    Seq[Loop] // where the first loop is the outer shape and the following ones are holes
  type MultiPolygon = Seq[Polygon]

  private[eliiza] def toS2MultiPolygon(
      multiPolygon: MultiPolygon
  ): S2Polygon = {
    def toS2Point(point: Point): S2Point =
      S2LatLng.fromDegrees(point(1), point(0)).toPoint
    def toS2Loop(loop: Loop): S2Loop = new S2Loop(loop.map(toS2Point).asJava)
  ...
  private[eliiza] def toMultiPolygon(multiPolygon: S2Polygon*): MultiPolygon = {
    def toPoint(point: S2Point): Point =
      Seq(longitude(point).degrees(), latitude(point).degrees())
    def toLoop(loop: S2Loop): Loop = for (i <- 0 until loop.numVertices())
      yield toPoint(loop.vertex(i))
  ...
  def multiPolygonArea(multiPolygon: MultiPolygon): Double = toS2MultiPolygon(
    multiPolygon
  ).getArea
  ...
  def intersectionArea(
    ...
    intersection.initToIntersection(
      toS2MultiPolygon(multiPolygonA),
      toS2MultiPolygon(multiPolygonB)
    )
